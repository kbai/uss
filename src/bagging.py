...
def Gridsearch_impl(X,Y,clf,param,cv):
    ...
    grid_search = GridSearchCV(clf,param,verbose=10,cv=cv,n_jobs=100)
    start = time()
    grid_search.fit(X,Y)
    best = report(grid_search.grid_scores_,10)
...
def main():
    ...
    Gridsearch_impl(X,Y,clf,param_grid,5)
#    for i in range(10,100,5):
#        clf = DecisionTreeClassifier(min_samples_split=i)
#        rf = RandomForestClassifier(n_estimators = 20000,random_state=0,min_samples_split=i)
